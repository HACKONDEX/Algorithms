...
bool topological_sort( Graph* graph, int* calls_order, unsigned char* colour, int& zeros_count, int start )
{
    std::stack< int > stack;
...
bool is_operation_executable( Graph* graph, int* calls_order )
{
    ...
    for( int i = 0; i < graph->get_vertex_count(); ++i )
    {
        ...
        {
            ++zeros_count;
            result = topological_sort( graph, calls_order, colour, zeros_count, i );
            if( !result )
                return false;
        }
    }
