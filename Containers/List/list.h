...
private:
    ...
    template<class... Args>
    ValueNode(Args&&... args) : value (std::forward<Args>(args)...){}
};
...
template<class T>
template<bool is_const>
typename List<T>::template List_iterator<is_const> &
List<T>::List_iterator<is_const>::operator=(const List<T>::List_iterator<false> &other) {
    node = other.node;
    return *this;
}
...
template<bool is_const>
typename List<T>::template List_iterator<is_const> &
List<T>::List_iterator<is_const>::operator=(const List<T>::List_iterator<true> &other) {
    node = other.node;
    return *this;
}
...
size_( size_ )
        ...
        Node* body = new Node();
        Node* prev = nullptr;
        body->value_node = new ValueNode( value );
        head->next = body;
        body->previous = head;
        prev = body;
...
void List<T>::copy(const List &other) {
    ...
    body->value_node = new ValueNode(other_body->value_node->value);
    head->next = body;
    body->previous = head;
    Node *prev = body;
    for (size_t i = 1; i < size_; ++i) {
...
typename List<T>::iterator List<T>::insert(List::const_iterator position, T &&value) {
    Node *body = new Node();
    body->value_node = new ValueNode(std::move(value));
    insert_before_position(position, body);
    return position;
}
...
void List<T>::emplace(List::const_iterator position, Args &&... args) {
    Node *body = new Node();
    body->value_node = new ValueNode(T(std::forward<Args>(args)...));
    insert_before_position(position, body);
}
