    ...
    for( int i = 0; i < vertices; ++i )
    {
        if( colour[i] == 0 )
        {
            ...
            while( !stack.empty() )
            {
                ...
                next_vertices = graph->get_next_vertices( current_node.vertex );
                if( colour[current_node.vertex] == 0 )
                {
                    colour[current_node.vertex] = 1;
                    low[current_node.vertex] = in_time[current_node.vertex] = timer++;
                    ...
                    for( int i = 0; i < next_vertices.size(); ++i )
                    {
                        ...
                        if( to == current_node.parent )
                            ...
                            stack.push( node( to, current_node.vertex ) );
                        else
                            low[current_node.vertex] = min( low[current_node.vertex], in_time[to] );
                    }
                }
                else
                {
                    stack.pop();
                    colour[current_node.vertex] = 2;

                    for( int i = 0; i < next_vertices.size(); ++i )
                    {
                        to = next_vertices[i];
                        if( current_node.parent != to  && low[to] < low[current_node.vertex] )
                            low[current_node.vertex] = low[to];
                    }
                    for( int i = 0; i < next_vertices.size(); ++i )
                    {
                        to = next_vertices[i];
                        if ( current_node.parent != to && low[to] > in_time[current_node.vertex])
                        {
                            if( matrix_graph[to][current_node.vertex] == 1 )
                            {
                                matrix_graph[to][current_node.vertex] = - 1;
                                matrix_graph[current_node.vertex][to] = - 1;
                            }
                        }
                    }
                }
...
bool find_any_cycle( Graph* graph, int** matrix_graph, int start, std::vector< int >& cycle )
{
    ...
    int cycle_begin = - 1; int cycle_end = - 1;
    int parent = 0; bool found_cycle = false;
...
void place_cycle_on_surface( Graph* g_plain, int** matrix_graph, bool* placed, std::vector< Area > & areas, std::vector< int >& cycle, std::vector< int >& placed_vertices )
{
    int j = 0;
    Area area_1;
    Area area_2;
    ...
    {
        g_plain->add_edge( cycle[j], cycle[j + 1] );
        g_plain->add_edge( cycle[j + 1], cycle[j] );
        placed[cycle[j]] = true;
    ...
    placed[cycle[j]] = true;
    placed_vertices.push_back( cycle[j] );
    g_plain->add_edge( cycle[j], cycle[0] );
    g_plain->add_edge( cycle[0], cycle[j] );
...
//Segment find_segment_from_contact_node( Graph* graph, Graph* g_plain, int** matrix_graph, bool* placed, bool* segmentated, std::vector< Area >& areas, int start )
//{
//Segment seg;
//std::queue< int > q;
//
...
Segment find_the_best_segment( Graph* graph, Graph* g_plain, int** matrix_graph, bool* placed, std::vector< Area >& areas, std::vector< int >& placed_vertices, bool* segmentated, bool& any_segment )
{
    Segment best_seg;
...
void get_chain_from_segment( Graph* graph, Segment& seg, bool* placed, std::vector< int >& chain )
{
    ...
    node v; bool break_from_dfs = false;
    stack.push( node( start, -1 ) );
...
void place_chain_on_surface( Graph* graph, Graph* g_plain, int** matrix_graph, std::vector< Area >& areas, int area_number, std::vector< int >& chain, bool* placed, std::vector< int >& placed_vertices )
{
        ...
        for (int i = 0; i < areas[area_number].boarder.size(); ++i)
        {
            ...
            else
            {
                ...
                for( j = 1; j < chain.size() - 1; ++j )
                {
                    ...
                    g_plain->add_edge( chain[j], chain[j - 1] );
                    g_plain->add_edge( chain[j - 1], chain[j] );
...
bool is_connection_component_planar( Graph* graph, int** matrix_graph, int any_vertex, bool* checked, bool* placed )
{
    ...
    else
    {
        ...
        Graph* g_plain = new Graph( verts_count );

        std::vector< Area > areas;
        int areas_count = 2;

        place_cycle_on_surface( g_plain, matrix_graph, placed, areas, cycle, placed_vertices );

        // time to find segments!!!
        // after segment was found recolour matrix_graph

        //print placed vertices
        while( placed_vertices.size() <= component.size() )// <=
        {
            bool any_segment = false;
            Segment best_segment = find_the_best_segment(graph, g_plain, matrix_graph, placed, areas, placed_vertices,
            ...
            // put chain on surface

            int area_number = best_segment.involved_areas[0];

            place_chain_on_surface(graph, g_plain, matrix_graph, areas, area_number, chain, placed, placed_vertices);
